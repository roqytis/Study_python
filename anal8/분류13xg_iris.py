...
import xgboost
model = xgboost.XGBClassifier(booster = 'gbtree', ma_depth = 4, )
# C : 모델에 페널티를 주면 정확도를 조정. tuning parameter
print(model)
...
print('분류 정확도 확인 3 :')
print('test  :', model.score(x_test, y_test)) # 모델을 사용하지 않고 분류 정확도 확인
print('train : ', model.score(x_train, y_train)) # 0.961904 이 값이 test결과와 차이가 많으면 오버피팅 또는 언더피팅 의심
...
# 만약 표준화하고 학습 했다면 얘도 표준화 해야함

new_pred = read_model.predict(new_data)

print('분류 결과 :', new_pred)
print('분류 결과(확률로 보기) :', read_model.predict_proba(new_data))
