...
# isis dataset을 사용
# train / data set으로 분리해 train으로 모델을 학습하고 test로 모델을 검정(평가) : overfitting(과적합) 방지
...
import xgboost
model = xgboost.XGBClassifier(booster = 'gbtree', max_depth = 4)
print(model)
...
print('예측값 : ', y_pred)
print('실제값 : ', y_test)
print('총 갯수:%d, 오류수:%d'%(len(y_test), (y_test != y_pred).sum()))  # 총 갯수:45, 오유수:1
print('분류 정확도 확인 1 :')
...
print('분류 정확도 확인 3 :')
print('test : ', model.score(x_test, y_test)) # 모델을 사용하지 않고 분류 정확도 확인  # 0.9777777
print('train : ', model.score(x_train, y_train)) # 0.961904  <- 이 값이 test 결과와 차이가 많으면 오버피팅 또는 언더피팅 의심
...
# [[1.25746592e-11 8.81678919e-04 9.99118321e-01]  -> 2
# [9.99999491e-01 5.09165576e-07 1.32715591e-21]  -> 0
# [5.23769533e-03 9.94762295e-01 9.41292546e-09]] -> 1
